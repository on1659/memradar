...
```css
--dashboard-activity-main:    minmax(0, 1.56fr);
--dashboard-activity-side-sm: 176px;
--dashboard-activity-side-lg: 258px;
```
...
**사용 카테고리** 는 `src/components/PersonalityView.tsx` 에 8종 정의. 새 유형을 추가할 때는 기존 8×8 체계와 중복되지 않게 축을 먼저 확인한다.
...
| 축 | 라벨 쌍 | 색상 | 의미 |
|---|---|---|---|
| style | 읽기형 ↔ 실행형 | `#7b6cf6` (violet) | Reader/Executor |
...
| 사용 카테고리 8종 | `src/components/PersonalityView.tsx` | 34-99 |

---

_최종 업데이트: 실제 코드(`v0.1.8` 기준)에서 역추출._ 새 UI 결정을 이 문서에 꾸준히 반영하고, 차이가 생기면 **코드가 정답**이라 가정하고 이 문서를 갱신한다.
