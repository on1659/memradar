...
```
1. Intro        — "당신의 이야기가 시작된 날"       (시간)
2. Prompts      — "1,234개의 프롬프트"             (정량)
3. Model        — "Your Favorite Model"            (선호)
4. Hours        — "Night Owl 🦉"                   (시간대 분석)
5. Personality  — "만능 빌더 — All-round Builder"  (성격)
6. Usage        — "풀스택 기획자"                   (사용 패턴)
7. Share        — "Made in 이더"                   (공유·퇴장)
```

현재 **실제 탑재된 슬라이드는 7장** (`WrappedView.tsx` 의 `slides` 배열, `lastSlideIndex = 6`). `slides/ToolsSlide.tsx` 파일은 저장소에 남아 있지만 어디에서도 `import` 되지 않는 **orphan** 이다. 도구 분석은 향후 고급 분석 영역이 생길 때까지 노출하지 않는다 (§10.1 참조).
...
**사용 카테고리** 는 `src/lib/usageProfile.ts` 에 **9종** 정의 (🧪 품질 감독관 포함). 렌더링 컴포넌트는 `src/components/PersonalityView.tsx` 의 `PersonalitySections`. 별도 라우트가 아니라 **Dashboard 가 `sectionMode="personality"` 로 렌더링** 한다 (`src/components/Dashboard.tsx`). 새 유형을 추가할 때는 `usageProfile.ts` 배열에 먼저 추가하고 기존 9종과 중복 여부를 확인한다.
