...
│   ├── codingRhythm.ts     # 코딩 리듬 — 로컬 날짜 키 per-day 집계 + 본인 분포 lift 기반 리듬 라벨 (6종, 약신호 시 null). 1순위(label)에 더해 다른 축(RHYTHM_FACET: time/weekday/density)의 2순위(secondaryLabel)를 가산(MIN_LABEL_LIFT 동일 게이트). 카드 본문 = 1순위 서사 + 2순위 경향 한 줄 + 하이라이트 칩 3종(활발 요일·최장 연속·밀도)
