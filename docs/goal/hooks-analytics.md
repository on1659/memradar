...
- **D5 Dashboard card.** Replaces TopSkillsCard in the same analytics-grid slot. Rename `.dashboard-analytics-card-skills` → `.dashboard-analytics-card-hooks` keeping grid-column rules and **min-height 13rem** (`src/index.css:264-267`). Title "훅 활동" / "Hook Activity"; ALL strings isKorean ternaries with BOTH branches. Renders exactly as its sibling analytics cards do: **title + donut**, nothing else. (1) title row = Webhook icon + "훅 활동"/"Hook Activity" + a CircleHelp `DashboardHoverTooltip` whose `title` carries the totals ("관측 N · 차단 N · 실패 N" / "Observed N · Blocked N · Failed N") and whose `description` carries the silent-allow caveat, plus the config pill inline at the end of the same row (`ml-auto`) so it costs zero height. (2) body = **`GenericDonutChart`** — the very component 세션 길이 / 사용한 모델 / 나 vs AI use — fed `[hookName, recordedExecutions][]`; it renders top-5 arcs + a "기타" arc and a legend (color dot · hookName · %), with `centerLabel` "훅"/"hooks" and the total in the middle. No bars, no inline rows, no per-script sub-rows, and the raw command string is never rendered anywhere on the card. Empty states collapse to a single centered `<p>` with the same footprint as the donut. *(Design history: the panel originally specced per-hookName rows with per-commandKey sub-rows showing the masked command. In live QA that produced ~1012px-wide `echo '{...}'` labels and ~10 sub-rows for the Stop group, inflating the card to 1452px and — via the analytics grid's `align-items:stretch` — padding the neighbouring 세션 길이 / 자주 쓴 단어 cards with whitespace. Collapsing to aggregate rows fixed the labels and got it to 448px, cutting rows to top-2 got it to 258px, and finally switching to the shared donut restored the sibling cards' exact "제목 + 도넛" shape and height while putting every hookName back on screen via the donut legend.)* Footer pill "설정 N개 · 관측 M개" opens an Escape-closable popover (theme-picker layer precedent) listing sanitized entries with provenance + confidence badges; pill hidden in upload mode. Empty states: (a) hook data → full card; (b) config + Claude sessions + zero observations → config summary + "기록된 실행 없음" explainer; (c) no Claude sessions OR no config source → `hasHookData` empty state with mode-appropriate copy. Same PR removals: `stats.topSkills`, computeStats skill tally, TopSkillsCard JSX, dead `src/components/TopSkills.tsx`. Keep violet accent for slot continuity. NO stacked bar segments in v1. *(Rejected: 3-zone card with 4-segment stacked bars + inline disclosure.)*
