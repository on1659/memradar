...
- **D5 Dashboard card.** Replaces TopSkillsCard in the same analytics-grid slot. Rename `.dashboard-analytics-card-skills` → `.dashboard-analytics-card-hooks` keeping grid-column rules and **min-height 13rem** (`src/index.css:264-267`). Title "훅 활동" / "Hook Activity"; ALL strings isKorean ternaries with BOTH branches. Two zones: (1) header = title + CircleHelp `DashboardHoverTooltip` (silent-allow caveat + metric definition) + chips "관측 N" always, "차단 N" amber / "실패 N" rose only when >0, sub-caption "가장 활발한 훅 · 기록 기준 · 외 N개"; (2) top-2 rows by recorded-execution count (row count matched to the neighbor analytics cards' natural height): single accent-color count bar + compact per-row outcome chips + hover tooltip full breakdown (incl. timedOut, summaryOnly, avgDurationMs scoped "기록 있는 실행 기준", lastSeen). Row label = hookName; **one aggregate row per hookName** (top-2), outcome chips summed across its commandKeys, with the **per-script breakdown moved into the hover tooltip** using short script basenames (e.g. `check-triage.sh`, `stop-review-gate-hook.mjs`) + per-script counts — NOT inline sub-rows and NEVER the full command string. never merge durations across commandKeys in the tooltip breakdown. Stop rows get a "전체 집계" tooltip marker. *(Revised post-panel during live QA: the original inline sub-row-per-commandKey design surfaced full masked commands in server mode — e.g. an `echo '{"systemMessage":"…"}'` label with ~1012px intrinsic width — and rendered unbounded sub-rows (the Stop group alone produced ~10), inflating the card to 1452px and, via the analytics grid's `align-items:stretch`, stretching neighboring cards to match. Collapsing to one aggregate row per hookName with `min-w-0` truncation and the script detail in the tooltip fixed the labels; the row count was then further cut 5→2 because 5 aggregate rows still made the card ~448px and `align-items:stretch` padded the neighbor analytics cards (session-length / word-cloud) with whitespace — 2 rows lands ~258px, matching them. Hidden groups surface via the "외 N개" caption + the config popover + per-session SessionView.)* Footer pill "설정 N개 · 관측 M개" opens an Escape-closable popover (theme-picker layer precedent) listing sanitized entries with provenance + confidence badges; pill hidden in upload mode. Empty states: (a) hook data → full card; (b) config + Claude sessions + zero observations → config summary + "기록된 실행 없음" explainer; (c) no Claude sessions OR no config source → `hasHookData` empty state with mode-appropriate copy. Same PR removals: `stats.topSkills`, computeStats skill tally, TopSkillsCard JSX, dead `src/components/TopSkills.tsx`. Keep violet accent for slot continuity. NO stacked bar segments in v1. *(Rejected: 3-zone card with 4-segment stacked bars + inline disclosure.)*
