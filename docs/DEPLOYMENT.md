...
Memradar 는 서버 없이 동작하는 정적 웹앱이다. 배포 전략도 그 전제에서 시작한다.

## TL;DR

**공식 배포는 Vercel 정적 배포.** 사용자는 URL 접속 후 로컬 `.jsonl` 파일을 직접 드래그·선택하거나, `npx memradar` 로 자동 스캔한다. 세션 로그는 서버로 업로드되지 않고 브라우저 안에서만 처리된다.
...
데모 사이트에 "샘플 보기" 버튼을 넣고 공개 가능한 `.jsonl` 픽스처를 fetch 해 로드한다. **실제 개인 로그로는 절대 사용하지 않는다** — 민감한 프롬프트·코드·경로가 유출될 수 있다.
